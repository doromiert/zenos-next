...
'''
    ...
    def test_dependency_cascades_are_ordered_assignment_statements(self) -> None:
        document = parse(
            "_meta.deps = { global = [ $pkgs.zenos.a ]; build ++ [ $pkgs.zenos.b ]; run -- [ $pkgs.zenos.a ]; run ++ [ $pkgs.zenos.c ]; };",
            "cascade.zpkg",
        )
    ...
    def test_rejects_actions_outside_zmdl_option_attrsets(self) -> None:
        ...
        for source in invalid:
            with self.subTest(source=source):
                ...
                self.assertEqual("ZEN203", raised.exception.diagnostic.code)
...
'''
    ...
    def test_enable_option_and_actions_do_not_gain_nested_context(self) -> None:
        ...
        with self.assertRaises(ZenLangError) as raised:
            parse('(freeform user) = { _meta.type = $type.bool; s! { x = true; }; };', "bad.zmdl")
        self.assertEqual("ZEN217", raised.exception.diagnostic.code)
    ...
    def test_zenos_version_metadata_uses_strict_version_literals(self) -> None:
        ...
        self.assertEqual("ZEN213", quoted.exception.diagnostic.code)
    ...
    def test_non_zpkg_assignment_cascades_are_rejected(self) -> None:
        ...
        for source in ("value ++ [ 1 ];", "_meta.deps = { run ++ true; };"):
            with self.subTest(source=source), self.assertRaises(ZenLangError) as raised:
                parse(source, "bad.zpkg")
            self.assertEqual("ZEN207", raised.exception.diagnostic.code)
...
class ImportGraphTests(unittest.TestCase):
    ...
    def test_dependency_records_scopes_and_operation_diagnostics(self) -> None:
        parse(
            '_meta.deps = { global = [ $pkgs.zenos.a { id = $pkgs.zenos.b; minVersion = "1.2"; } ]; export ++ [ $pkgs.zenos.c ]; };',
            "deps.zpkg",
        )
        invalid = (
            "_meta.deps = { global = true; };",
            "_meta.deps = { other = [ $pkgs.zenos.a ]; };",
            "_meta.deps = { global = [ $pkgs.a ]; };",
            "_meta.deps = { run -- [ $pkgs.zenos.absent ]; };",
            "_meta.deps = { global = [ $pkgs.zenos.one.same $pkgs.zenos.two.same ]; };",
        )
        for source in invalid:
            with self.subTest(source=source), self.assertRaises(ZenLangError) as raised:
                parse(source, "bad.zpkg")
    ...
    def test_import_path_failures_and_depth_have_stable_diagnostics(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            entry.write_text('_import "broken.zcfg";', encoding="utf-8")
            original_resolve = Path.resolve

            def fail_broken_path(path: Path, *args: object, **kwargs: object) -> Path:
                if path.name == "broken.zcfg":
                    raise OSError("cannot resolve test path")
                return original_resolve(path, *args, **kwargs)

            with patch("zenlang.api.Path.resolve", new=fail_broken_path):
                with self.assertRaises(ZenLangError) as raised:
                    parse_file(entry)
    ...
    def test_final_component_symlink_replacement_is_not_followed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            def replace_before_open(path: Path | str, flags: int, *, dir_fd: int | None = None) -> int:
                nonlocal replaced
                if Path(path).name == resolved_entry.name and dir_fd is not None and not replaced:
                    replaced = True
                    entry.unlink()
                    entry.symlink_to(replacement)
                return original_open(path, flags, dir_fd=dir_fd)

            with patch("zenlang.api.os.open", new=replace_before_open):
                with self.assertRaises(ZenLangError) as raised:
                    parse_file(entry)
...
class CliTests(unittest.TestCase):
    ...
    def test_cli_renders_imported_source_and_compiles_zcfg(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("zenRuntime.buildPackage descriptor", stdout.getvalue())
